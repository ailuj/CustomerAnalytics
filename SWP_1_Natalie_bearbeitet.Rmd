...
\textbf{Ishvarya Siddharth}, 584210; 
\textbf{Julia Dullin}, 587779; \\
Submitted on November 13, 2017 

\end{center}

1. The data set contains data about the salaries of professors. In total, there are 6 variables and 397 observations in the data set. The variables are _rank_, _discipline_, _yrs.since.phd_, _yrs.service_, _sex_ and _salary_. 
```{r, echo=FALSE}
library(car)
library(corrplot)
data <- Salaries
...
dimensions <- dim(data)
```
2. There are 19 professors who have more than 40 years of service, assuming the term _professor_ refers only to observations with _rank == 'Prof'_
```{r}
...
salary
```
4. The mean salary for professors with more than 20 years of service with the rank restricted to _rank == 'Prof'_ is
```{r}
...
summary(data)
```
6. The following tables 1 and 2 provide the counts and proportions of men and women by rank. The results show that the number of male professors is significantly higher than the number of women.  
```{r, echo=FALSE}
...
7. The following figure 1 provides a histogram for _years of service_. The number of professors decreases the more time they have been in service.

```{r fig1, fig.cap="Histogram of years of service for both men and women", fig.align="center", fig.width=5,fig.height=4}
...
9. The salary for professors seperated by rank is visualized in Fig.3. The observations with the rank _Prof_ have the highest salary followed by _AssocProf_ and _AsstProf_. _Prof_ is also the only rank with outliers on the upper end.

```{r fig3, echo=FALSE, fig.cap="Boxplot for salary of professors by rank",fig.width=5,fig.height=6}
...
11. The correlation for salary vs. years since PhD and for salary vs. years of service are shown in the following table. Between Years since PhD and salary is a higher correlation than between years of service and salary. 
...
12. The Pearson's product-moment correlation shows the statistical significance. The correlation between _years since PhD_ and _salary_ is 0.4192311, which indicates a weak to moderate positive relationship, whereas the correlation between _years of service_ and _salary_ is 0.3347447, which indicates a weak positive relationship.
...
13. The last figure displays all bivariate relationships of the given dataset "salaries.csv" and visualizes the results of 12.
