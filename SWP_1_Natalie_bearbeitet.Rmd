...
Group 9: Julia , Ishvarya Siddhat, Natalie Habib 
...
8. Fig. 2 shows the boxplot for the variable _salary_ for all observed professors.  
```{r fig2, echo=TRUE, fig.cap="Boxplot for salary of professors, including all observations"}
...
9. The salary for professors seperated by rank visualize Fig. 3. The observations with the rank _Prof_ have on avergae the highest salary followed by _AssocProf_ and _AsstProf_.
```{r fig3, echo=TRUE, fig.cap="Boxplot for salary of professors by rank"}
...
```{r fig4, echo=TRUE, fig.cap="Plot which shows the salary of all observations vs years since PhD"}
...
11. The correlation for salary vs. years since PhD and for salary vs. years of service are shown in the following table. 
```{r}
num_data <- data[, sapply(data, is.numeric)]
cor(num_data) #correlation cor.test()
```

12. The correlation shows the statistically significance. 
```{r}
...
```{r fig5, echo=TRUE, fig.cap="Visualization of all bivariate relationships"}
