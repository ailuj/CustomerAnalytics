...
Calculating the mean salaries by rank and sex shows that male university employees with the rank “Professor“ have the highest mean with 127,120.82 Euros, followed by the male “Association Professor“ with 94,869.70 Euros and the male “Assistant Professor“ with 81,311.46 Euros. Male employee’s mean is for all ranks higher than female employee’s mean, the salary for male employees is always above the mean and female's is always below the mean salary for each rank.

```{r mean.salaries, echo=FALSE, fig.cap="Tab. 1: Mean Salaries by sex and rank"}
...
```{r salary.boxplot, echo=FALSE, fig.cap="Fig. 1: Displaying the salary distribution by sex and rank"}
...
```{r femaleByDiscipline, echo=FALSE, fig.cap="Tab. 2: Proportion for woman by discipline"}
...
The mean salary for men and women among university employees is different. The one-way ANOVA displayed in Table 3 shows that the F value is 7.74 and the p-value is low, less than 0.05. Hence, the conclusion is that the alternative hypothesis is accepted, the means of salaries for men and women are not equal, they are different.

```{r anova, echo=FALSE, fig.cap="Fig. 2: Displaying the results of the one-way ANOVA test"}
...
```{r anovaGraph, echo=FALSE, fig.cap="Fig. 3: Testing and visualizing the ANOVA results on statistical signicficance"}
...
```{r transformationSalary, fig.cap="Fig. 4: Plotting the salary of all observations"}
...
```{r linearModel, fig.cap="Fig. 5: Displaying the results of the linear model"}
fit <- lm(formula = salary ~ sex + rank + discipline + yrs.service,
          data = salaries) 
summary(fit)
```

Plot the coefficients for those effects.

```{r coefficientPlot, fig.cap="Fig. 6: Visualization of the coefficinets of the linear model"}
